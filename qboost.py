...
class DecisionStumpClassifier:
    ...
    def __init__(self, X, y, feature_index):
        ...
        """
        self.i = feature_index #Teaches clf using only one specific feature
...
class EnsembleClassifier:
    ...
    def precision_calc(self, X, y):
        """Compute accuracy score on given data."""
        if sum(self.w) == 0:
        # Avoid difficulties that occur with handling this below
            return 0.0
        return precision_score(y, self.predict_class(X))

    def recall_calc(self, X, y):
        """Compute accuracy score on given data."""
        if sum(self.w) == 0:
            # Avoid difficulties that occur with handling this below
            return 0.0
        return recall_score(y, self.predict_class(X))

    def f1_calc(self, X, y):
        """Compute accuracy score on given data."""
        if sum(self.w) == 0:
            # Avoid difficulties that occur with handling this below
            return 0.0
